...
# This is an analysis of a random sample dataframe of 2189 individuals sourced from the 2017 British Election Study data. The 2189 individuals were classified on Gender into Male and Female. Their responses on use of Facebook and Twitter respectively were entered as binary nominal variables. 

# Main findings
# 1. Men are 31% less likely than women to use Facebook. 
# 2. Women are 23.5% less likely than men to use Twitter.
# 3. Those who use Twitter are more than 7 times more likely to be Facebook users as well than those who do not use Twitter. 
# 4. The likeliest person in our sample to be a Facebook user is a Twitter-using Female (90.6% predicted probability) and the least likely person to be a Facebook user is a non-Twitter-using Male (45% predicted probability).
...
# Logistic regression: Predicting gender from social media use
...
# A binary logistic regression was conducted to predict the likelihood of being Female based on Twitter and Facebook usage. Women were 23.5% less likely than men to use Twitter (OR = 0.77, 95% CI: 0.60 - 0.97, p < 0.001). Women were 53.8% more likely than men to use Facebook (OR = 1.54, 95% CI: 1.29 - 1.84, p < 0.001). This suggests Facebook-only users have the highest probability of being Female, while Twitter-only users have the highest probability of being Male. Those who use neither platform are closer to an even gender split.